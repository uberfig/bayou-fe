...
use crate::{api::{methods::communities::get_comm_rooms::community_rooms, types::auth_token::AuthToken}, state::{State, AUTH_TOKEN}};
...
pub fn CommunityRoomsBar() -> impl IntoView {
    ...
    let resource = move || LocalResource::new(move || {
        let token = logged_in.get_untracked().expect("trying to get comm rooms when not logged in");
        let state = state.get_untracked();
        let community = id();
        community_rooms(state, token, community)
    });
    let (rooms, _set_rooms) = signal(resource());
    let render_rooms = move || {
        let rooms = rooms.get();
        match rooms.get() {
            Some(Ok(rooms)) => {
